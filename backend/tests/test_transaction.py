...
from backend.config.config import MINING_REWARD_INPUT, MINING_REWARD
...
def test_transaction_update():
    ...
    to_first_again = 25
    transaction.update(sender_wallet, first_recipient, to_first_again)

    assert transaction.output[first_recipient] == first_amount + to_first_again
    assert transaction.output[
               sender_wallet.address] == sender_wallet.balance - first_amount - next_amount - to_first_again
    assert Wallet.verify(
        transaction.input['public_key'],
        transaction.output,
        transaction.input['signature']
