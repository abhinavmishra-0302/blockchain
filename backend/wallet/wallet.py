...
class Wallet:
    ...
    def verify(public_key, data, signature):
        ...
        try:
            deserialized_public_key.verify(encode_dss_signature(r, s), json.dumps(data).encode('utf-8'),
    ...
    def calculate_balance(blockchain, address):
        """
        Calculate the balance of the given address considering the transaction data
        in the blockchain.
...
def main():
    ...
    print(f'should_be-valid: {should_be_valid}')

    wallet2 = Wallet()

    should_be_invalid = Wallet.verify(wallet2.public_key, data, signature)
